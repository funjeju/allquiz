...
import { getStoredMonthlyQuiz, getMonthlyQuizKey } from "@/services/quizService";
...
export default function MonthlyQuizPage() {
  ...
  useEffect(() => {
    getStoredMonthlyQuiz().then(data => {
      const list = data ?? [];
      setQuestions(list);
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
      ...
      <p className="font-black text-primary uppercase tracking-widest animate-pulse">Loading Monthly Quiz...</p>
    </div>
  );
  ...
  if (questions.length === 0) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background px-6 text-center gap-6">
      ...
      <h2 className="text-2xl font-black">Monthly Quiz 준비 중</h2>
      <p className="text-muted-foreground">지난달 퀴즈가 아직 생성되지 않았습니다.<br />매월 1일에 자동으로 생성됩니다.</p>
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full">
        <div className="bg-card border-2 border-primary/20 p-8 rounded-[48px] shadow-2xl text-center">
          <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest mb-5">
            <CalendarDays className="w-4 h-4" /> Monthly Quiz
          </div>
          <h1 className="text-4xl font-black mb-1 tracking-tighter">Monthly Quiz</h1>
          <p className="text-muted-foreground font-medium mb-6">{monthKey} 월간 뉴스 퀴즈 모음</p>
          <div className="grid grid-cols-2 gap-3 mb-6 text-left">
            ...
            <div className="bg-muted/50 p-4 rounded-2xl border border-border">
              ...
              <p className="text-sm font-black text-primary">지난달 Daily</p>
            </div>
          </div>
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full">
          <div className="bg-card border-2 border-primary/30 p-8 rounded-[48px] shadow-2xl text-center">
            <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest mb-4">
              <CalendarDays className="w-4 h-4" /> Monthly Result
            </div>
      ...
      <div className="max-w-2xl w-full">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-black text-muted-foreground uppercase tracking-widest">Monthly Quiz</span>
