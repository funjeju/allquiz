...
export default function DailyQuizPage() {
  ...
  // ── READY ──────────────────────────────────────────────────
  if (gameState === "READY") {
    const myRank = user ? battleScores.findIndex(s => s.uid === user.uid) + 1 : 0;
    return (
          ...
          <div className="bg-card border-2 border-primary/20 p-8 rounded-[48px] shadow-2xl mb-4">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6 text-left">
              <div className="bg-muted/50 p-3 rounded-2xl border border-border">
                ...
                <p className="text-xl font-black text-primary">{questions.length}</p>
              </div>
              <div className="bg-muted/50 p-3 rounded-2xl border border-border">
                <p className="text-[10px] font-black text-muted-foreground uppercase mb-1">참여자</p>
                <p className="text-xl font-black text-primary">{battleScores.length}</p>
              </div>
              <div className="bg-muted/50 p-3 rounded-2xl border border-border">
                <p className="text-[10px] font-black text-muted-foreground uppercase mb-1">내 순위</p>
                <p className="text-xl font-black text-primary">{myRank > 0 ? `${myRank}위` : "-"}</p>
              </div>
            </div>
            <motion.button
              ...
              <div className="space-y-2">
                {battleScores.slice(0, 5).map((s, i) => (
                  <div key={s.uid} className={`flex items-center gap-3 p-2.5 rounded-2xl ${s.uid === user?.uid ? "bg-primary/10 border border-primary/20" : "bg-muted/30"}`}>
                    ...
                    <span className="flex-1 text-sm font-bold truncate">{s.uid === user?.uid ? "나" : s.nickname}</span>
                    <span className="text-sm font-black text-primary">{s.score}/{s.total}</span>
                    <span className="text-xs font-bold text-muted-foreground">{s.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  if (gameState === "RESULT") {
    ...
    const myRank = battleScores.findIndex(s => s.uid === user?.uid) + 1;

    return (
      <div className="min-h-screen bg-background p-6 flex flex-col items-center justify-center">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full space-y-4">
          <div className="bg-card border-2 border-primary/30 p-8 rounded-[48px] shadow-2xl text-center">
            ...
            <p className="text-xs font-black text-muted-foreground uppercase tracking-widest mb-1">오늘의 결과</p>
            <div className="text-6xl font-black tracking-tighter mb-1">{grade}</div>
            <div className="text-xl font-black text-primary mb-4">{score} / {questions.length}문제 정답</div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              <div className="bg-muted/50 p-3 rounded-2xl border border-border">
                <p className="text-[9px] font-black text-muted-foreground uppercase mb-1">정답률</p>
                <p className="text-lg font-black text-primary">{pct}%</p>
              </div>
              <div className="bg-muted/50 p-3 rounded-2xl border border-border">
                <p className="text-[9px] font-black text-muted-foreground uppercase mb-1">획득 포인트</p>
                <p className="text-lg font-black text-accent">+{savedPoints ?? score * 10}</p>
              </div>
              <div className="bg-muted/50 p-3 rounded-2xl border border-border">
                <p className="text-[9px] font-black text-muted-foreground uppercase mb-1">배틀 순위</p>
                <p className="text-lg font-black text-primary">{myRank > 0 ? `${myRank}위` : "-"}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button onClick={() => router.push("/me")} className="w-full bg-card border border-border py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 text-muted-foreground text-sm">
                나의 기록 보기
              </button>
          ...
          {battleScores.length > 0 && (
            <div className="bg-card border border-border rounded-3xl p-5">
              <h3 className="text-xs font-black text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                <Swords className="w-3.5 h-3.5 text-primary" /> 오늘의 배틀 랭킹
              </h3>
              <div className="space-y-2">
                {battleScores.slice(0, 10).map((s, i) => (
                  <div key={s.uid} className={`flex items-center gap-3 p-3 rounded-2xl ${s.uid === user?.uid ? "bg-primary/10 border border-primary/20" : "bg-muted/30"}`}>
                    <span className={`text-sm font-black w-5 ${i === 0 ? "text-yellow-400" : i === 1 ? "text-gray-400" : i === 2 ? "text-amber-600" : "text-muted-foreground"}`}>{i + 1}</span>
                    <span className="flex-1 text-sm font-bold truncate">{s.uid === user?.uid ? `${s.nickname} (나)` : s.nickname}</span>
                    <span className="text-sm font-black text-primary">{s.score}/{s.total}</span>
                    <span className="text-xs font-bold text-accent">{s.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
